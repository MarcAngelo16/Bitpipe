        ...
        if get_args().enable_bitpipe_schedule:
            ...
            for j in range(num_bd_parallel_groups):
                ...
                if rank in bd_ranks:
                    ...
                    print(f"[BitPipe BD Group] Rank {rank} assigned to BD group {j}: {bd_ranks}", flush=True)
            
            # Log summary of all BD groups (only from rank 0 to avoid duplication)
            if rank == 0:
                print(f"[BitPipe BD Groups Summary] Total {num_bd_parallel_groups} BD groups created:", flush=True)
                for idx, group_ranks in enumerate(all_bd_groups):
                    print(f"[BitPipe BD Groups Summary]   BD Group {idx}: {group_ranks}", flush=True)
