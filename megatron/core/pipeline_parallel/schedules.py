...
def get_forward_backward_func():
    ...
    """
    # Chimera 2-VR schedule (higher priority than BitPipe)
    if hasattr(get_args(), 'enable_chimera_schedule') and get_args().enable_chimera_schedule:
        print(f"[DEBUG] Chimera 2-VR schedule ENABLED", flush=True)
    ...
    # BitPipe 4-VR schedule
    if get_args().enable_bitpipe_schedule:
        print(f"[DEBUG] BitPipe 4-VR schedule ENABLED - using new organized structure", flush=True)
...
def forward_step(
    ...
    print(f"[forward_step] Calling set_input_tensor with: type={type(input_tensor)}, is_list={isinstance(input_tensor, list)}, content=[{type(input_tensor[0]) if input_tensor else 'empty'}]", flush=True)
    set_input_tensor(input_tensor)
    print(f"[forward_step] set_input_tensor done", flush=True)
...
def forward_backward_pipelining_without_interleaving(
    *,
    ...
    # Initialize profiler if enabled for current iteration
    from megatron.core.iteration_context import should_profile_current_iteration
    
    profiler = None
    if should_profile_current_iteration():
        from megatron.core.pipeline_parallel.bitpipe_profiler import initialize_bitpipe_profiler
        import torch
        world_size = torch.distributed.get_world_size() if torch.distributed.is_initialized() else 1
        rank = parallel_state.get_pipeline_model_parallel_rank()
        profiler = initialize_bitpipe_profiler(rank, world_size, enabled=True, schedule_type="1f1b")
        profiler.start_profiling()
        print_rank_0("Standard 1F1B profiling enabled")

    # Helper function for P2P profiling
    def _profile_p2p_if_enabled(p2p_func, comm_type, *args, **kwargs):
        """Conditionally wrap P2P calls with BitPipe profiler while keeping Megatron timers"""
        from megatron.core.pipeline_parallel.bitpipe_profiler import get_bitpipe_profiler
        bitpipe_profiler = get_bitpipe_profiler()
        if bitpipe_profiler and bitpipe_profiler.enabled:
            from megatron.core.pipeline_parallel.bitpipe_schedule import _profile_p2p_comm
            return _profile_p2p_comm(p2p_func, comm_type, *args, **kwargs)
        else:
            return p2p_func(*args, **kwargs)
    ...
    for i in range(num_warmup_microbatches):
        ...
        input_tensor = _profile_p2p_if_enabled(recv_forward, "recv_forward", recv_tensor_shapes, config)
        
        # Start profiling forward pass (after receiving input tensor)
        if profiler:
            profiler.start_microbatch(i, 0, rank, 'forward')  # Standard 1F1B: pipeline_id=0, model_chunk_id=rank
        output_tensor = forward_step(
            forward_step_func,
            data_iterator,
            model,
        ...
        _profile_p2p_if_enabled(send_forward, "send_forward", output_tensor, send_tensor_shapes, config)
        
        # End profiling forward pass
        if profiler:
            profiler.end_microbatch(i, 0, rank, 'forward')
    ...
    # Run 1F1B in steady state.
    if profiler and num_microbatches_remaining > 0:
    ...
    for i in range(num_microbatches_remaining):
        ...
        # Start profiling forward pass
        if profiler:
            profiler.start_microbatch(forward_microbatch_id, 0, rank, 'forward')
        ...
        # End profiling forward pass
        if profiler:
            profiler.end_microbatch(forward_microbatch_id, 0, rank, 'forward')
        ...
        if forward_only:
            ...
            output_tensor_grad = _profile_p2p_if_enabled(send_forward_recv_backward, "send_forward_recv_backward",
            ...
            # Start profiling backward pass
            if profiler:
                profiler.start_microbatch(backward_microbatch_id, 0, rank, 'backward')
                
            input_tensor_grad = backward_step(
                input_tensor, output_tensor, output_tensor_grad, model_type, config
            )
            
            # End profiling backward pass
            if profiler:
                profiler.end_microbatch(backward_microbatch_id, 0, rank, 'backward')

            if last_iteration:
                input_tensor = None
                _profile_p2p_if_enabled(send_backward, "send_backward", input_tensor_grad, recv_tensor_shapes, config)
            else:
                input_tensor = _profile_p2p_if_enabled(send_backward_recv_forward, "send_backward_recv_forward",
    ...
    if not forward_only:
        ...
        for i in range(num_warmup_microbatches):
            cooldown_microbatch_id = i + num_microbatches_remaining
            ...
            output_tensor_grad = _profile_p2p_if_enabled(recv_backward, "recv_backward", send_tensor_shapes, config)

            # Start profiling backward pass (after receiving gradient)
            if profiler:
                profiler.start_microbatch(cooldown_microbatch_id, 0, rank, 'backward')
            ...
            # End profiling backward pass
            if profiler:
                profiler.end_microbatch(cooldown_microbatch_id, 0, rank, 'backward')
    ...
    # Save profiling data for standard 1F1B
    if profiler:
