"""
BitPipe: 4-VR Bidirectional Interleaved Pipeline Parallelism (Symmetric)

This module implements the original BitPipe schedule with 4 virtual ranks (VR) per device,
forming a V-shaped execution pattern for optimal pipeline utilization.
...
Migrated from: megatron/core/pipeline_parallel/bitpipe_schedule.py
New location: megatron/core/pipeline_parallel/schedules/bitpipe/bitpipe_4vr.py
"""
...
# P2P Communication profiling wrappers
def _profile_p2p_comm(comm_func, comm_type, *args, **kwargs):
    ...
    # Execute the actual communication
...
def print_all_ranks(message):
    """Print from all ranks for debugging BitPipe schedule."""
    # Force printing if BITPIPE_DEBUG is set
    if os.environ.get('BITPIPE_DEBUG', '0') == '1' or True:  # Always print for now
        if torch.distributed.is_initialized():
            rank = torch.distributed.get_rank()
            world_rank = os.environ.get('RANK', 'unknown')
            print(f"[BitPipe Rank {rank}/World {world_rank}] {message}", flush=True)
        else:
            print(f"[BitPipe] {message}", flush=True)
...
def forward_backward_pipelining_with_bitpipe_4vr(
    *,
    ...
    # # Log that BitPipe is being used
    # print_all_ranks("=== ENTERING BitPipe forward_backward_pipelining_with_BitPipe ===")
    ...
    if should_profile_current_iteration():
        ...
        # Determine schedule type based on whether asymmetric is enabled
        args = get_args()
        schedule_type = "bitpipe_asym" if getattr(args, 'enable_bitpipe_asymmetric', False) else "bitpipe"
        profiler = initialize_bitpipe_profiler(pipeline_parallel_rank, world_size, enabled=True, schedule_type=schedule_type)
        profiler.start_profiling()
    
    # # Log BitPipe configuration
    # print_all_ranks("\n========== BitPipe Schedule Configuration ==========")
    # print_all_ranks(f"Pipeline parallel size: {pipeline_parallel_size}")
    # print_all_ranks(f"Pipeline parallel rank: {pipeline_parallel_rank}")
    # print_all_ranks(f"Number of microbatches (base): {num_microbatches}")
    # print_all_ranks(f"Number of model chunks: {len(model)}")
    ...
    # # Log phase breakdown
    # print_all_ranks(f"Total microbatches (doubled): {total_num_microbatches}")
    # print_all_ranks(f"n_loop: {n_loop}")
    # print_all_ranks(f"Phase breakdown:")
    # print_all_ranks(f"  - Warmup microbatches: {num_warmup_microbatches}")
    # print_all_ranks(f"  - Mid-phase microbatches: {num_microbatches_mid}")
    # print_all_ranks(f"  - Remaining microbatches: {num_microbatches_remaining}")
    # print_all_ranks(f"  - Unit remaining: {unit_remaining}")
    # print_all_ranks("==================================================\n")
    ...
    def get_microbatch_idx(total_num_microbatches, pipeline_parallel_rank):
        ...
        # # Log forward schedule
        # print_all_ranks(f"Forward schedule (length={len(microbatch_idx)}): {microbatch_idx}")
        # print_all_ranks("Forward pass execution order:")
        # for time_step, mb_id in enumerate(microbatch_idx):
        #     chunk_id = get_model_chunk_id(mb_id)
        #     print_all_ranks(f"  Time {time_step}: Process microbatch {mb_id} using model chunk {chunk_id}")
    ...
    def get_bkmicrobatch_idx(total_num_microbatches, pipeline_parallel_rank):
        ...
        # # Log backward schedule
        # print_all_ranks(f"Backward schedule (length={len(microbatch_idx)}): {microbatch_idx}")
        # print_all_ranks("Backward pass execution order:")
        # for time_step, mb_id in enumerate(microbatch_idx):
        #     if mb_id == -1:
        #         print_all_ranks(f"  Time {time_step}: GRADIENT SYNC")
        #     else:
        #         chunk_id = get_model_chunk_id(mb_id)
        #         print_all_ranks(f"  Time {time_step}: Process microbatch {mb_id} using model chunk {chunk_id}")
    ...
    def forward_step_helper(microbatch_id, checkpoint_activations_microbatch,offset):
        ...
        # Pipeline ID based on model chunk:
        # Pipeline 0: Model chunks 0 and 2 (forward direction flow)
        # Pipeline 1: Model chunks 1 and 3 (backward direction flow)
            ...
            for k in range(unit_remaining*2):
                ...
                # Forward pass.
                    ...
                    if not parallel_state.is_pipeline_last_stage():# recv grad if not last stage
                        output_tensor_grad= _profile_p2p_comm(
