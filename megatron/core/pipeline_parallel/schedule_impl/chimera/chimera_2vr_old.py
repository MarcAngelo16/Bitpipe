    ...
    *,
    ...
    if not forward_only and num_1f1b_microbatches > 0:
        ...
        for k in range(num_1f1b_microbatches):
            ...
            if need_send_bwd and need_recv_fwd:
                ...
                first_cooldown_mb = microbatch_idx_b[bwd_idx]
                ...
                if can_bridge:
                    ...
                    if bwd_model_chunk_id == 0 and first_cool_vr == 0:
                        # VR0 send_prev + VR0 recv_next
                        bridged_grad = p2p_communication.chimera_grad_send_prev_recv_next(
                            input_tensor_grad, tensor_shape, config
                        )
                    elif bwd_model_chunk_id == 0 and first_cool_vr == 1:
                        # VR0 send_prev + VR1 recv_prev
                        bridged_grad = p2p_communication.chimera_grad_send_prev_recv_prev(
                            input_tensor_grad, tensor_shape, config
                        )
                    elif bwd_model_chunk_id == 1 and first_cool_vr == 0:
                        # VR1 send_next + VR0 recv_next
                        bridged_grad = p2p_communication.chimera_grad_send_next_recv_next(
                            input_tensor_grad, tensor_shape, config
                        )
                    else:  # bwd_model_chunk_id == 1 and first_cool_vr == 1
                        # VR1 send_next + VR1 recv_prev
                        bridged_grad = p2p_communication.chimera_grad_send_next_recv_prev(
                            input_tensor_grad, tensor_shape, config
                        )
                    ...
                    # Fall back to standalone send.
                    print_all_ranks(
                        f"[Rank{pipeline_parallel_rank}] Send bwd only VR{bwd_model_chunk_id} "
                    ...
                    print_all_ranks(f"[Rank{pipeline_parallel_rank}] Sent bwd grad (fallback)")
    ...
    if not forward_only:
        ...
        while bwd_idx < len(microbatch_idx_b):
            ...
            # Check if the bridge already pre-fetched this gradient
            ...
            need_recv = not cool_is_grad_first and not have_bridged_grad
            ...
            if have_bridged_grad:
                ...
                bridged_cooldown_vr = None
                print_all_ranks(
                    f"[Rank{pipeline_parallel_rank}] Using BRIDGED grad for VR{model_chunk_id} "
                    f"MB{microbatch_id} (no recv needed)"
                )

                # If there's also a pending send, flush it now (standalone)
                if have_pending_send:
                    print_all_ranks(f"[Rank{pipeline_parallel_rank}] Flushing pending grad VR{pending_grad_vr} alongside bridged recv")
                    if pending_grad_vr == 0:
                        p2p_communication.chimera_grad_send_prev_only(pending_grad_to_send, config)
                    else:
                        p2p_communication.chimera_grad_send_next_only(pending_grad_to_send, config)
