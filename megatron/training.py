...
def pretrain(train_valid_test_dataset_provider,
    ...
    if args.do_valid:
        # Set iteration context for final validation
        from megatron.core.iteration_context import set_iteration_context
        set_iteration_context(1, 'eval')  # Final validation as eval iteration 1
        
        prefix = f'iteration {iteration} on validation set'
        # Note: evaluate_and_print_results function needs to be implemented
    ...
    if args.do_test:
        # Set iteration context for final test
        from megatron.core.iteration_context import set_iteration_context
        set_iteration_context(1, 'eval')  # Final test as eval iteration 1
    ...
    if mpu.get_pipeline_model_parallel_world_size() > 1 and \
       args.virtual_pipeline_model_parallel_size is not None:
        ...
        for i in range(args.virtual_pipeline_model_parallel_size):
            ...
            # ========== DEBUG LOG: Show pre_process and post_process flags ==========
            # Log which flags each VR chunk receives for easier verification
            # Print from ALL ranks to see complete picture across all devices
            pipeline_rank = mpu.get_pipeline_model_parallel_rank()
            schedule_type = "BitPipe 4-VR" if args.enable_bitpipe_schedule else \
                           "Chimera 2-VR" if (hasattr(args, 'enable_chimera_schedule') and args.enable_chimera_schedule) else \
                           "Standard"

            # Print from all pipeline ranks to see all devices
            print(f"[VR_INIT] {schedule_type:12s} | Device {pipeline_rank} | VR {i} | "
                  f"pre_process={str(pre_process):5s} | post_process={str(post_process):5s}",
                  flush=True)
            # =========================================================================
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    while iteration < args.train_iters:
        ...
        # Set iteration context for BitPipe profiling (1-indexed)
        from megatron.core.iteration_context import set_iteration_context
        set_iteration_context(iteration + 1, 'train')
        ...
        if args.eval_interval and iteration % args.eval_interval == 0 and \
           args.do_valid:
            # Set iteration context for evaluation (calculate eval iteration number)
            eval_iteration = (iteration // args.eval_interval) 
            from megatron.core.iteration_context import set_iteration_context
            set_iteration_context(eval_iteration, 'eval')
