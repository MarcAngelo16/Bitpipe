...
def load_and_validate_asymmetric_config(config_path, pipeline_size, total_layers):
    """Load and validate asymmetric configuration from JSON file"""
    # Load JSON file
    try:
        with open(config_path, 'r') as f:
            config = json.load(f)
    except Exception as e:
        raise ValueError(f"Failed to load asymmetric config from {config_path}: {e}")
    
    # Extract configuration
    if 'first_half_devices' not in config:
        raise ValueError("JSON must contain 'first_half_devices' key")
    
    first_half_devices = config['first_half_devices']
    
    # Validation 1: Pipeline size must be even
    if pipeline_size % 2 != 0:
        raise ValueError(f"BitPipe requires even number of devices, got {pipeline_size}. "
                        f"Supported sizes: 2, 4, 6, 8, 10, 12, ...")
    
    # Validation 2: Must provide exactly pipeline_size//2 device configs
    expected_devices = pipeline_size // 2
    actual_devices = len(first_half_devices)
    if actual_devices != expected_devices:
        raise ValueError(f"For {pipeline_size} devices, expected {expected_devices} "
                        f"device configurations in first_half_devices, got {actual_devices}")
    
    # Validation 3-5: Validate each device
    for i, device_config in enumerate(first_half_devices):
        # Must have exactly 4 VR values
        if len(device_config) != 4:
            raise ValueError(f"Device {i} must have exactly 4 VR values, got {len(device_config)}")
        
        # All values must be integers
        if not all(isinstance(v, int) for v in device_config):
            raise ValueError(f"Device {i} has non-integer values: {device_config}")
        
        # No empty chunks (minimum 1 layer)
        if any(v < 1 for v in device_config):
            raise ValueError(f"Device {i} has empty chunks: {device_config}. Each chunk needs ≥1 layer")
    
    # Validation 6: Total layers must match
    total_input_layers = sum(sum(dev) for dev in first_half_devices)
    if total_input_layers != total_layers:
        raise ValueError(f"Total layers in config ({total_input_layers}) != model layers ({total_layers})")
    
    # Generate full configuration using pairing logic
    ...
    print(f"✓ BitPipe Asymmetric Configuration Loaded:")
    ...
    for i in range(pipeline_size):
        ...
        if i < pipeline_size // 2:
            print(f"  Device {i}: {full_config[i]} (pairs with Device {pair_idx})")
        else:
            print(f"  Device {i}: {full_config[i]} (paired from Device {pair_idx})")
...
def validate_args(args, defaults={}):
    ...
    if args.enable_bitpipe_schedule:
        ...
        if args.enable_bitpipe_asymmetric:
            ...
            if not args.bitpipe_asymmetric_config:
                raise ValueError("--bitpipe-asymmetric-config must be specified when "
                               "--enable-bitpipe-asymmetric is set")
            
            # Load and validate asymmetric configuration
            args.asymmetric_layers_full_config = load_and_validate_asymmetric_config(
                args.bitpipe_asymmetric_config,
                args.transformer_pipeline_model_parallel_size,
                args.num_layers
            )
            
            # Skip symmetric validations for asymmetric mode
            print("BitPipe asymmetric mode enabled - bypassing symmetric layer requirements")
        else:
            # Original symmetric validations
            assert (
    ...
    # Chimera 2-VR schedule configuration
    if hasattr(args, 'enable_chimera_schedule') and args.enable_chimera_schedule:
        args.virtual_pipeline_model_parallel_size = 2
        if args.rank == 0:
            print(f"[CHIMERA INIT] Setting virtual_pipeline_model_parallel_size = 2")

        if hasattr(args, 'enable_chimera_asymmetric') and args.enable_chimera_asymmetric:
            # Ensure config file is provided
            if not args.chimera_asymmetric_config:
            ...
            # For Chimera 2-VR: each device gets (num_layers / pipeline_size) layers per VR
            num_layers_per_device = (
                args.num_layers // args.transformer_pipeline_model_parallel_size
            )

            # Store for later use
            args.num_layers_per_chimera_vr = num_layers_per_device

            if args.rank == 0:
                print(f"[CHIMERA INIT] Symmetric mode:")
                print(f"  - Pipeline parallel size: {args.transformer_pipeline_model_parallel_size}")
                print(f"  - Total layers: {args.num_layers}")
                print(f"  - Layers per device: {num_layers_per_device}")
                print(f"  - Layers per VR: {num_layers_per_device} (2 VRs per device)")
...
def _add_distributed_args(parser):
    ...
    group.add_argument('--bitpipe-asymmetric-config', type=str, default=None,
                       help='Path to JSON file containing asymmetric layer distribution. '
                            'Required when --enable-bitpipe-asymmetric is set.',
                       dest='bitpipe_asymmetric_config')

    # Chimera 2-VR pipeline arguments
    group.add_argument('--enable-chimera-schedule', action='store_true',
                       help='Use Chimera 2-VR bidirectional pipeline (simplified alternative to BitPipe 4-VR).',
                       dest='enable_chimera_schedule')
    group.add_argument('--enable-chimera-asymmetric', action='store_true',
                       help='Enable asymmetric layer distribution for Chimera 2-VR',
                       dest='enable_chimera_asymmetric')
    group.add_argument('--chimera-asymmetric-config', type=str, default=None,
                       help='Path to JSON file containing asymmetric layer distribution for Chimera. '
                            'Required when --enable-chimera-asymmetric is set.',
                       dest='chimera_asymmetric_config')
