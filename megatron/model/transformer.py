...
def _get_num_layers(args, model_type, is_decoder=False):
    ...
    if model_type == ModelType.retro_encoder:
        ...
        if is_encoder_and_decoder_model:
            ...
            # STEP 1: Distribute base layers evenly across all pipeline stages
            # ================================================================
            # Special case: standalone embedding stage puts embedding on rank 0 only,
            # so rank 0 VRs get 0 layers (embedding only), other ranks split remaining layers.
            # Normal case: divide total layers equally among pipeline stages.
            num_layers = (
                0
                if args.standalone_embedding_stage
                and mpu.get_pipeline_model_parallel_rank() == 0 else
            ...
            # DEBUG OUTPUT: Show base layer distribution
            if mpu.get_data_parallel_rank() == 0:
                print(f"[LAYER_CALC] Input: args.num_layers={args.num_layers}, pipeline_stages={args.transformer_pipeline_model_parallel_size}")
                print(f"[LAYER_CALC] Result: Each device gets {num_layers} base layers")

            # STEP 2: Double layers for interleaved pipeline schedules
            # ========================================================
            # BitPipe (4-VR) and Chimera (2-VR) use interleaved bidirectional execution.
            # This requires doubling the total layers in the model so that when divided
            # among Virtual Ranks (chunks), each VR still has enough work to do.
            #
            # Example: 24 base layers, 4 devices, BitPipe 4-VR:
            #   Base per device: 24/4 = 6 layers
            #   After doubling:  6*2 = 12 layers per device (6 for forward path, 6 for backward path)
            #   Per VR:          12/4 = 3 layers per VR
            if args.enable_bitpipe_schedule:
                num_layers = num_layers * 2
                if mpu.get_data_parallel_rank() == 0:
                    print(f"[LAYER_CALC] BitPipe enabled: doubled to {num_layers} layers per device")
                    print(f"[LAYER_CALC]   → Will be divided into 4 VRs: {num_layers//4} layers per VR")

            if hasattr(args, 'enable_chimera_schedule') and args.enable_chimera_schedule:
                num_layers = num_layers * 2
                if mpu.get_data_parallel_rank() == 0:
                    print(f"[LAYER_CALC] Chimera enabled: doubled to {num_layers} layers per device")
                    print(f"[LAYER_CALC]   → Will be divided into 2 VRs: {num_layers//2} layers per VR")
    else:
        if not is_decoder:
...
class ParallelTransformer(MegatronModule):
    ...
    def __init__(self, config,
        ...
        if config.virtual_pipeline_model_parallel_size is not None:
            # =====================================================================
            # INTERLEAVED PIPELINE OFFSET CALCULATION
            # =====================================================================
            # This section handles layer assignment for THREE different scheduling strategies:
            # 1. STANDARD INTERLEAVED: Layers distributed interleaved across VRs
            # 2. BITPIPE 4-VR: Complex V-shaped bidirectional pipeline with 4 VRs per device
            # 3. CHIMERA 2-VR: Simplified bidirectional pipeline with 2 VRs per device
            #
            # All three strategies follow the same layer calculation flow:
            # - STEP 1 (in _get_num_layers): Divide base layers by pipeline stages
            # - STEP 2 (in _get_num_layers): Double layers for interleaved schedules
            # - STEP 3 (below): Divide by VR count to get per-VR layer count
            # - STEP 4 (below): Calculate offset using scheduler-specific logic
            # - STEP 5 (in __init__): Create transformer layers from offset to offset+num_layers
            #
            # The key difference is in the offset calculation algorithm:
            # - Standard: offset = vp_rank * (total_layers/vp_size) + pipeline_rank * num_layers_per_vr
            # - BitPipe:  offset = complex V-shaped formula with pairing and halving
            # - Chimera:  offset = simple sequential + device-swap pattern
            #
            # IMPORTANT CONSTANTS at this point:
            # - config.num_layers = total layers for all VRs on this device (after doubling)
            ...
            # =====================================================================
            # STEP 3: Divide layers by number of Virtual Ranks (chunks per device)
            # =====================================================================
            # At this point self.num_layers contains the total layers for this device
            # (after optional doubling for interleaved schedules).
            #
            # For interleaved schedules, we need to divide by the number of VRs/chunks:
            # - BitPipe 4-VR: Each device has 4 Virtual Ranks, so divide by 4
            # - Chimera 2-VR: Each device has 2 Virtual Ranks, so divide by 2
            # - Standard pipeline: No VRs, so divide by 1 (no change)
            #
            # This gives us the number of layers for THIS SPECIFIC VR/CHUNK.
            #
            # Example: Chimera 2-VR with 24 base layers, 4 devices
            #   After STEP 1 (distribution): 24/4 = 6 layers per device
            #   After STEP 2 (doubling):      6*2 = 12 layers per device
            #   After STEP 3 (VR division):   12/2 = 6 layers per VR
            #
            # Example: BitPipe 4-VR with 16 base layers, 4 devices
            #   After STEP 1 (distribution): 16/4 = 4 layers per device
            #   After STEP 2 (doubling):      4*2 = 8 layers per device
            #   After STEP 3 (VR division):   8/4 = 2 layers per VR
            #
            # Standard (non-interleaved) layer distribution for reference:
            # With 8 layers, 2 stages, and 4 model chunks:
            #   Stage 0: [0]  [2]  [4]  [6]  (interleaved)
            #   Stage 1: [1]  [3]  [5]  [7]  (interleaved)
            #
            # Contiguous layer distribution (used for standard pipeline):
            # With 8 layers, 2 stages, and 2 virtual stages:
            #   Stage 0: [0, 1]  [4, 5]  (contiguous)
            #   Stage 1: [2, 3]  [6, 7]  (contiguous)
            self.num_layers = self.num_layers // config.virtual_pipeline_model_parallel_size

            # Now compute the offset (starting layer index) for this device-VR combination
            # For standard interleaved pipeline (no BitPipe/Chimera):
            offset = mpu.get_virtual_pipeline_model_parallel_rank() * (
                config.num_layers // config.virtual_pipeline_model_parallel_size) + \
                (mpu.get_pipeline_model_parallel_rank() * self.num_layers)
            # =====================================================================
            # BitPipe 4-VR LAYER ASSIGNMENT
            # =====================================================================
            # BitPipe uses a complex V-shaped bidirectional pipeline with 4 Virtual Ranks
            # per device. Each device processes layers in two pairs of VRs:
            # - VR0-VR1: Forward direction (first half of model)
            # - VR2-VR3: Backward direction (second half of model)
            #
            # Layer distribution pattern with V-shape for 16 layers, 4 devices:
            ...
            #
            # At this point: self.num_layers = 2 (16 base/4 devices/2 = 2 per VR)
            if args.enable_bitpipe_schedule:
                assert config.virtual_pipeline_model_parallel_size == 4, \
                    f"BitPipe requires virtual_pipeline_model_parallel_size=4, got {config.virtual_pipeline_model_parallel_size}"

                if args.enable_bitpipe_asymmetric:
                    # -----------------------------------------------
                    # ASYMMETRIC MODE: Custom layer distribution
                    # -----------------------------------------------
                    # Use user-provided asymmetric configuration for heterogeneous GPU clusters
                    ...
                    offset, self.num_layers = get_asymmetric_offset(
                        ...
                        args.asymmetric_layers_full_config
                    )
                else:
                    # -----------------------------------------------
                    # SYMMETRIC MODE: V-shaped layer assignment
                    # -----------------------------------------------
                    # The V-shape pattern ensures that bidirectional microbatches have
                    # similar computational load. Layers are mapped as follows:
                    #
                    # VR INDEX MAP:
                    # VR0 and VR2 are "forward" chunks (accessed in sequence: 0→1→2→3)
                    # VR1 and VR3 are "backward" chunks (accessed in sequence: 3→2→1→0)
                    #
                    # KEY INSIGHT: VR indices are mapped to device positions:
                    # vp_idx = 0 if vp_rank < 2 else (vp_rank-1) % 2
                    # - VR0 (vp_rank=0): vp_idx=0 (forward pattern)
                    # - VR1 (vp_rank=1): vp_idx=0 (backward pattern, paired with VR0)
                    # - VR2 (vp_rank=2): vp_idx=1 (forward pattern, second half)
                    # - VR3 (vp_rank=3): vp_idx=1 (backward pattern, paired with VR2)

                    vp_idx = mpu.get_virtual_pipeline_model_parallel_rank() \
                        if mpu.get_virtual_pipeline_model_parallel_rank() < 2 \
                        else (mpu.get_virtual_pipeline_model_parallel_rank() - 1) % 2

                    # OFFSET CALCULATION:
                    # offset = base_offset + pairing_offset + half_offset
                    #
                    # 1. base_offset = pipeline_rank * self.num_layers
                    #    Assigns sequential layers to devices for forward direction
                    #
                    # 2. pairing_offset = vp_idx * (pipeline_size - 1 - 2*pipeline_rank) * self.num_layers
                    #    For vp_idx=0: Creates normal forward ordering
                    #    For vp_idx=1: Creates V-shape by reversing the device ordering
                    #
                    # 3. half_offset = (vp_rank//2) * (total_layers//2)
                    #    Separates first half (VR0,VR1) from second half (VR2,VR3)
                    ...
                    offset = (
                        ...
                        + (mpu.get_virtual_pipeline_model_parallel_rank() // 2) * (config.num_layers // 2)
                    )
            # =====================================================================
            # CHIMERA 2-VR LAYER ASSIGNMENT
            # =====================================================================
            # Chimera uses a SIMPLIFIED bidirectional interleaved pipeline with only
            # 2 Virtual Ranks per device instead of BitPipe's 4. This reduces complexity
            # while maintaining the bidirectional execution benefits.
            #
            # Layer assignment pattern with sequential + device-swap for 24 layers, 4 devices:
            #   Device 0: VR0[1-6]    VR1[19-24]  (VR1 gets Device 3's VR0 layers)
            #   Device 1: VR0[7-12]   VR1[13-18]  (VR1 gets Device 2's VR0 layers)
            #   Device 2: VR0[13-18]  VR1[7-12]   (VR1 gets Device 1's VR0 layers)
            #   Device 3: VR0[19-24]  VR1[1-6]    (VR1 gets Device 0's VR0 layers)
            #
            # KEY DIFFERENCES from BitPipe:
            # - Only 2 VRs per device (not 4): Simpler scheduling logic
            # - Sequential device ordering (not V-shaped): Simpler communication pattern
            # - Direct bidirectional pairing: Device i ↔ Device (N-1-i) share same layers
            # - No same-device VR transitions: Each microbatch stays in same VR throughout journey
            #
            # At this point: self.num_layers = 6 (24 base/4 devices/2 = 6 per VR)
        ...
        if self.num_layers == 0:
            ...
            layer_numbers = [i + 1 + offset for i in range(self.num_layers)]
            print(f"[BITPIPE LOG] Device {mpu.get_pipeline_model_parallel_rank()} | Virtual Rank {mpu.get_virtual_pipeline_model_parallel_rank()} | Created layers: {layer_numbers}")
    ...
    def forward(self, hidden_states, attention_mask,
        ...
        if not self.pre_process:
            ...
            # input_tensor might be a list (from forward_step wrapping), so unwrap it
            if isinstance(self.input_tensor, list):
                hidden_states = self.input_tensor[0] if self.input_tensor else None
                print(f"[Transformer] Non-first stage: unwrapped list input_tensor, hidden_states type={type(hidden_states)}", flush=True)
            else:
                hidden_states = self.input_tensor
                print(f"[Transformer] Non-first stage: direct input_tensor, hidden_states type={type(hidden_states)}", flush=True)
        else:
            print(f"[Transformer] First stage: using function parameter, hidden_states type={type(hidden_states)}", flush=True)
        ...
        print(f"[Transformer] Before make_viewless_tensor: hidden_states type={type(hidden_states)}, is_tensor={isinstance(hidden_states, torch.Tensor)}, value={'None' if hidden_states is None else 'tensor'}", flush=True)
